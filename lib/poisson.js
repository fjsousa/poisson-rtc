...
Poisson.prototype.solver = function (maxItt, maxRes) {
  ...
  if ( res <= maxRes ) {
    ...
    return 'Solver didn\'t converged. Residue is' +  res + 'at iteration ' + itt ;
  }
}
...
Poisson.prototype.setBoundaryConditions = function(N, S, E, W) {
  ...
  for (var col = 0; col < N.length; col ++) {
    this.u.old[col] = N[col];
    this.u.old[(this.n -1) * this.m + col] = S[col];
  }

  for (var row = 0; row < E.length; row ++ ){
    this.u.old[row*this.n] = W[row];
    this.u.old[row*this.n + this.n - 1] = E[row];
